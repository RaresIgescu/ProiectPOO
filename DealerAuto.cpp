...
public:
    Banca(string nume, float dobanda, float tva, vector <int> rate) {
        Nume = nume;
        Dobanda = dobanda;
        TVA = tva;
        NumarRate = rate;
    ...
    void GetRate() {
        for(int i = 0; i < NumarRate.size(); i++)
            cout << NumarRate[i] << " ";
    }

    float CalculPretFinal(float pret) {
    ...
    float GetRataLunara(float pret) {
        ...
        int Indice;
        for(int i = 0; i < NumarRate.size(); i++)
            cout << ++NrRata << ". " << NumarRate[i] << "\n";
        cout << "Alegeti indecele numarului de rate." << "\n";
        cin >> Indice;
        return pret/NumarRate[Indice-1];
    }
...
public:
    Dealer_Auto() {
        ...
        Flota.push_back(Masina("FERRARI", "812GTS", 282983.93, 2023 ));
        ...
        Flota.push_back(Masina("PORSCHE", "911GT3", 32981.92, 2007 ));
        Flota.push_back(Masina("PORSCHE", "PANAMERA", 32981.92 ));
        BanciPartenere.push_back(Banca("ING", 3, 19, {84, 120, 156, 192}));
        BanciPartenere.push_back(Banca("BRD", 2, 10, {60, 120, 180}));
        BanciPartenere.push_back(Banca("BT", 5, 8, {36, 60, 84, 120, 180}));
    }
    ...
    void Cumpara() {
        ...
        while (functionare)
        {
            ...
            for(int i = 0; i < Flota.size(); i++)
                if(Flota[i].GetMarca() == MarcaAleasa)
                    cout << ++NrModel << "." << Flota[i].GetModel() << "\n";
            cout << "Va rugam alegeti cifra corespunzatoare modelului:" << "\n";
            cin >> InputModel;
            while(InputModel<1 || InputModel > 6)
            {
                NrModel = 0;
                cout << "Nu ati ales corect, reincercati!" << "\n";
                for (int i = 0 ; i < Flota.size(); i++)
            ...
            for(int i = 0; i < Flota.size(); i++)
                if(Flota[i].GetMarca() == MarcaAleasa and Flota[i].GetModel() == ModelAles)
                    PretInitial = Flota[i].GetPret();
            ...
            if (InputContinuare == 1)
            {
                ...
                else
                {
                    ...
                    NrBanca = 0;
                    for(int i = 0; i < BanciPartenere.size(); i++)
                        cout << ++NrBanca << ". " << BanciPartenere[i].GetNume()<< "\n";
                    cin >> InputContinuare;
    ...
    void VizualizareFlota() {
        for(int i = 0; i < Flota.size(); i+=3)
        {
            ...
            for(int j = 0; j<Flota.size();j++)
                if(Flota[j].GetMarca() == Flota[i].GetMarca())
                    cout << ++NrModel << ". " << Flota[j].GetModel() << "\n";
            cout << "\n";
        }
    }
